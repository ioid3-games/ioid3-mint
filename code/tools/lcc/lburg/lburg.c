...
static void emitcase(Term p, int ntnumber) {
	...
	for (r = p->rules; r; r = r->next) {
		...
		case 1:
			...
			if (r->pattern->nterms == 2 && r->pattern->left && r->pattern->right == NULL)
		...
		case 2:
			if (r->pattern->nterms > 1) {
				print("%2if (%1/* %R */\n", r);
				emittest(r->pattern->left, "LEFT_CHILD(a)", r->pattern->right->nterms ? " && " : " ");
...
static void emitrecalc(char *pre, Term root, Term kid) {
	...
	if (root->kind == TERM && strncmp(root->name, "INDIR", 5) == 0 && kid->kind == TERM && strcmp(kid->name, "VREGP") == 0) {
		Nonterm p;
...
"%s%1p->cost[%P%S_NT] = %s + %d;\n%s%1p->rule.%P%S = %d;\n", c, cost, r->lhs, pre, r->lhs, c, cost, pre, r->lhs, r->packed);
...
static void emitstring(Rule rules) {
	...
	for (r = rules; r; r = r->link) {
		...
		print("/* %d */%1%d, %1/* %s */\n", r->ern, len >= 2 && r->template[len - 2] == '\\' && r->template[len - 1] == 'n', r->template);
	}
...
static void emittest(Tree t, char *v, char *suffix) {
	...
	if (p->kind == TERM) {
		print("%3%s->op == %d%s/* %S */\n", v, p->esn, t->nterms > 1 ? " && " : suffix, p);

		if (t->left)
			emittest(t->left, stringf("LEFT_CHILD(%s)", v), t->right && t->right->nterms ? " && " : suffix);
