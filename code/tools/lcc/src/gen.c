...
static void prelabel(Node p) {
	...
	case CVI: case CVU: case CVP:
		if (optype(p->op) != F && opsize(p->op) <= p->syms[0]->u.c.v.i)
...
void rtarget(Node p, int n, Symbol r) {
	...
	if (r != q->syms[RX] && !q->syms[RX]->x.wildcard) {
		q = newnode(LOAD + opkind(q->op), q, NULL, q->syms[0]);
...
Node gen(Node forest) {
	...
	for (p = forest; p; p = p->x.next) {
		...
		if (p->x.listed && NeedsReg[opindex(p->op)] && (*IR->x.rmap) (opkind(p->op))) {
...
static int uses(Node p, Regnode rn) {
	...
	for (i = 0; i < NELEMS(p->x.kids); i++)
		if (
			p->x.kids[i] && p->x.kids[i]->x.registered && rn->set == p->x.kids[i]->syms[RX]->x.regnode->set && (rn->mask&p->x.kids[i]->syms[RX]->x.regnode->mask)
	)
			return 1;
	return 0;
}
static void spillr(Symbol r, Node here) {
	...
	while (p->x.prevuse)
		assert(r == p->syms[RX]), p = p->x.prevuse;
...
void spill(unsigned mask, int n, Node here) {
	...
	if (mask&~freemask[n]) {
		assert(/* It makes no sense for a node to clobber() its target. */
			...
			here->syms[RX]->x.regnode->set != n || (here->syms[RX]->x.regnode->mask&mask) == 0
	);
