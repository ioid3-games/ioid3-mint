...
static void bbcall(Symbol yycounts, Coordinate *cp, Tree *e) {
	...
	t = tree(t->op, t->type, tree(RIGHT, t->kids[0]->type, t->kids[0], tree(RIGHT, t->kids[0]->type, asgn(caller, idtree(p)), t->kids[0])), t->kids[1]);
...
static void bbincr(Symbol yycounts, Coordinate *cp, Tree *e) {
	...
	t = incr('+', rvalue((*optree['+']) (ADD, pointer(idtree(yycounts)), consttree(npoints++, inttype))), consttree(1, inttype));
