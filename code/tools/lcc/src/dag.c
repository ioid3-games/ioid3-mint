...
static void kill(Symbol p) {
	...
	for (i = 0; i < NELEMS(buckets); i++)
		for (q = &buckets[i]; *q;)
			if (generic((*q)->node.op) == INDIR && (!isaddrop((*q)->node.kids[0]->op) || (*q)->node.kids[0]->syms[0] == p)) {
				*q = (*q)->hlink;
				--nodecount;
			} else
				q = & (*q)->hlink;
...
Node listnodes(Tree tp, int tlab, int flab) {
	...
	case RIGHT: {if (tp->kids[0] && tp->kids[1]
			 ...
			 && tp->kids[0]->kids[0] == tp->kids[1]->kids[0]) || (tp->kids[0]->op == FIELD
	...
	case ASGN:  {assert(tlab == 0 && flab == 0);
			...
			if (fieldsize(f) < 8 * f->type->size) {
				...
				if ((q->op == CNST + I && q->u.v.i == 0) || (q->op == CNST + U && q->u.v.u == 0))
					...
					listnodes(q, 0, 0);
					q = bittree(BOR, bittree(BAND, rvalue(lvalue(x)), cnsttree(unsignedtype, (unsigned long)~mask)), bittree(BAND, shtree(LSH, cast(q, unsignedtype), cnsttree(unsignedtype, (unsigned long)fieldright(f))), cnsttree(unsignedtype, (unsigned long)mask)));
				}
...
void gencode(Symbol caller[], Symbol callee[]) {
	...
	for (; errcnt <= 0 && cp; cp = cp->next)
		switch (cp->kind) {
		case Address: (*IR->address) (cp->u.addr.sym, cp->u.addr.base, cp->u.addr.offset); break;
		case Blockbeg: {
...
static Node prune(Node forest) {
	...
	for (p = forest; p; p = p->link) {
		...
		if ((generic(p->op) == ASGN && generic(p->kids[0]->op) == ADDRL && p->kids[0]->syms[0]->temporary && p->kids[0]->syms[0]->u.t.cse == p->kids[1])) {
			...
			if ((generic(p->kids[1]->op) == INDIR
			 && isaddrop(p->kids[1]->kids[0]->op) && p->kids[1]->kids[0]->syms[0]->sclass == REGISTER) || ((generic(p->kids[1]->op) == INDIR
			 && isaddrop(p->kids[1]->kids[0]->op)) && tmp->sclass == AUTO) || (generic(p->kids[1]->op) == ADDRG && tmp->sclass == AUTO)) {
				tmp->u.t.replace = 1;
				count++;
