...
static void decl(Symbol(*dcl)(int, char *, Type, Coordinate *)) {
	...
	if (t == ID || t == '*' || t == '(' || t == '[') {
		...
		if (level == GLOBAL) {
			...
			if (params && id && isfunc(ty1) && (t == '{' || istypename(t, tsym) || (kind[t] == STATIC && t != TYPEDEF))) {
				if (sclass == TYPEDEF) {
...
static Type dclr1(char **id, Symbol **params, int abstract) {
	...
	case '*': t = gettok(); if (t == CONST || t == VOLATILE) {
					Type ty1;
					ty1 = ty = tnode(t, NULL)' || ((t = gettok()) == CONST || t == VOLATILE)
						ty1 = tnode(t, ty1);
...
static void funcdefn(int sclass, char *id, Type ty, Symbol params[], Coordinate pt) {
	...
	if (Aflag >= 2 && sclass != STATIC && strcmp(id, "main") == 0) {
		...
		else if (!(rty == inttype && ((n == 0 && callee[0] == NULL) || (n == 2 && callee[0]->type == inttype && isptr(callee[1]->type) && callee[1]->type->type == charptype && !variadic(ty)))))
