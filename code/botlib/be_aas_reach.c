...
int AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity) {
	...
	teststart[2] += 64;
	trace = AAS_TracePlayerBBox(teststart, origin, PRESENCE_CROUCH, -1, BOTMASK_SOLID);
...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		AAS_PlayerMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL, qfalse, qfalse, velocity, cmdmove, 0, 30, 0.1f, bboxmins, bboxmaxs, bot_visualizejumppads, BOTMASK_SOLID);
...
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs, vec3_t goalorigin) {
	...
	if (areanum) {
		...
		trace = AAS_TracePlayerBBox(start, end, PRESENCE_CROUCH, -1, BOTMASK_SOLID);
		...
		if (!trace.startsolid) {
			...
			// an AAS_TracePlayerBBox from that point will return trace.startsolid qtrue
#if 0
...
Returns true if there is a solid just after the end point when going from start to end.
=======================================================================================================================================
*/
...
Searches for reachabilities between adjacent areas with equal floor heights.
=======================================================================================================================================
*/
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num) {
	...
	if (foundreach) {
		...
		if (AAS_AreaGroundFaceArea(lreach->areanum) < 500) {
			lreach->traveltime += 100;
		}
		*/
		reach_equalfloor++;
		...
		for (k = 0; k < groundface1->numedges; k++) {
			...
			for (j = 0; j < area2->numfaces; j++) {
				...
				for (l = 0; l < groundface2->numedges; l++) {
					...
					// project the two ground edges into the step side plane and calculate the shortest distance between the two
					// edges if they overlap in the direction orthogonal to the gravity direction
					...
					if (groundface1->faceflags & FACE_GROUND) {
						// if the vertical distance is smaller, or the vertical distance is pretty much the same but the overlapping part of the edges is longer
						if (dist < ground_bestdist || (dist < ground_bestdist + 1 && length > ground_bestlength)) {
						...
						// if the vertical distance is smaller, or the vertical distance is pretty much the same but the overlapping part of the edges is longer
						if (dist < water_bestdist || (dist < water_bestdist + 1 && length > water_bestlength)) {
							water_bestdist = dist;
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		testend[2] -= 100;
		trace = AAS_TracePlayerBBox(teststart, testend, PRESENCE_NORMAL, -1, BOTMASK_SOLID);
		...
		for (i = 0; i < 3; i++) {
			...
			// movement prediction
			AAS_PredictPlayerMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 3, 30, 0.1f, stopevent, 0, qfalse, BOTMASK_SOLID);
			// if prediction time wasn't enough to fully predict the movement
			if (move.frames >= 30) {
				return qfalse;
			}
			// never jump or fall through a cluster portal and don't enter slime or lava
			if (move.stopevent & (SE_TOUCHCLUSTERPORTAL|SE_ENTERSLIME|SE_ENTERLAVA)) {
				return qfalse;
			}
...
*/
		...
		for (i = 0; i < 3; i++) {
			...
			VectorScale(dir, speed, velocity);
			// movement prediction
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, qtrue, velocity, cmdmove, 3, 30, 0.1f, stopevent, 0, qfalse);
			// if prediction time wasn't enough to fully predict the movement
			if (move.frames >= 30) {
				return qfalse;
			}
...
int AAS_Reachability_Ladder(int area1num, int area2num) {
	...
	if (ladderface1 && ladderface2) {
		...
		// if both vertical ladder faces, and the ladder faces do not make a sharp corner, and the shared edge is not too vertical
		if (ladderface1vertical && ladderface2vertical && DotProduct(plane1->normal, plane2->normal) > 0.7 && fabsf(DotProduct(sharededgevec, up)) < 0.7) {
			// create a new reachability link
			lreach = AAS_AllocReachability();
		...
		if (ladderface1vertical) {
			...
			// if from another area without vertical ladder faces, the reachabilities shouldn't exist already
			if (i >= area2->numfaces && area2num != area1num && !AAS_ReachabilityExists(area1num, area2num) && !AAS_ReachabilityExists(area2num, area1num)) {
...
Create possible teleporter reachabilities, this is very game dependent.... :(
...
//#endif REACH_DEBUG
		...
		for (dest = AAS_NextBSPEntity(0); dest; dest = AAS_NextBSPEntity(dest)) {
			// classname should be misc_teleporter_dest but I've also seen target_position and actually any entity could be used... burp
			if (AAS_ValueForBSPEpairKey(dest, "targetname", targetname, MAX_EPAIRKEY)) {
				if (!strcmp(targetname, target)) {
					break;
				}
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
			...
			trace = AAS_TracePlayerBBox(destorigin, end, PRESENCE_CROUCH, -1, BOTMASK_SOLID);
			...
			if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num) && !AAS_AreaGrounded(area2num)) {
			...
			} else*/
			{
				...
				VectorClear(cmdmove);
				// movement prediction
				AAS_PredictPlayerMovement(&move, -1, destorigin, PRESENCE_NORMAL, qfalse, qfalse, velocity, cmdmove, 0, 30, 0.1f, SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER, 0, qfalse, BOTMASK_SOLID);
...
void AAS_Reachability_Elevator(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		if (!strcmp(classname, "func_plat")) {
			...
			// get the mins, maxs and origin of the model
			// NOTE: the origin is usually (0, 0, 0) and the mins and maxs are the absolute mins and maxs
		...
		*/
		...
		for (i = 0; i < 2; i++) {
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach) {
				...
				//trace = AAS_TracePlayerBBox(startreach->start, move_start_top, PRESENCE_NORMAL, -1, BOTMASK_SOLID);

				//if (trace.fraction < 1) continue;

				for (endreach = firstendreach; endreach; endreach = nextendreach) {
					nextendreach = endreach->next;
					//trace = AAS_TracePlayerBBox(endreach->end, move_end_top, PRESENCE_NORMAL, -1, BOTMASK_SOLID);
...
//		VectorScale(velocity, speed, velocity);
		...
		*/
		...
		if (velocity[0] || velocity[1]) {
			...
			for (i = 0; i < 20; i++) {
				// movement prediction
				AAS_PredictPlayerMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, qfalse, velocity, cmdmove, 0, 30, 0.1f, SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER, 0, bot_visualizejumppads, BOTMASK_SOLID);
		...
		for (area2num = 1; area2num < aasworld.numareas; area2num++) {
			...
			*/
			// never try to go back to one of the original jumppad areas and don't create reachabilities if they already exist
			for (link = areas; link; link = link->next_area) {
				if (AAS_ReachabilityExists(link->areanum, area2num)) {
					break;
				}
			...
			for (i = 0; i < area2->numfaces; i++) {
				...
				if (ret && speed < 150) {
					...
					//if (hordist < 1.6 * facecenter[2] - areastart[2])
					{
						...
						// movement prediction
						AAS_PredictPlayerMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, qfalse, velocity, cmdmove, 30, 30, 0.1f, SE_ENTERWATER|SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER|SE_HITGROUNDAREA, area2num, visualize, BOTMASK_SOLID);
...
Never point at ground faces. Aalways a higher and pretty far area.
=======================================================================================================================================
*/
...
void AAS_SetWeaponJumpAreaFlags(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		if (!strcmp(classname, "item_armor_body") ||
	...
	for (i = 0; i < area2->numfaces; i++) {
		...
		for (n = 0; n < 1/*2*/; n++) {
			...
			if (ret && speed < 300) {
				...
				// movement prediction
				AAS_PredictPlayerMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue, qfalse, velocity, cmdmove, 30, 30, 0.1f, SE_ENTERWATER|SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_TOUCHJUMPPAD|SE_HITGROUND|SE_HITGROUNDAREA, area2num, visualize, BOTMASK_SOLID);
			...
			for (j = 0; j < area->numfaces; j++) {
				...
				for (l = 0; l < face2->numedges; l++) {
					...
					if (abs(edge1num) == abs(edge2num)) {
						...
						testend[2] -= 1000;
						trace = AAS_TracePlayerBBox(mid, testend, PRESENCE_CROUCH, -1, BOTMASK_SOLID);
...
int AAS_ContinueInitReachability(float time) {
	...
	for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++) {
		...
		if (botimport.MilliSeconds() - start_time > (int)reachability_delay) {
			break;
		}
