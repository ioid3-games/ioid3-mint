...
//	void (*CG_Refresh)(int serverTime, stereoFrame_t stereoView, qboolean demoPlayback, connstate_t state, int realTime);
	// draws UI and if connected to a server, generates and draws a game scene and status information at the given time.
...
//	char * (*CG_VoIPString)(void);
	// pass voip target token unknown by client to cgame to convert into playerNums use Cmd_Argc()/Cmd_Argv() to read the
	// target token, return a string of comma-delimited playerNums based on target token or NULL if unknown token
	CG_KEY_EVENT,
...
//	void (*CG_ConsoleText)(int realTime, qboolean restoredText);
	// pass text that has been printed to the console to cgame, use Cmd_Argc()/Cmd_Argv()to read it
