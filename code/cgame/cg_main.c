...
void CG_AddNotifyText(int realTime, qboolean restoredText) {
	...
	buffer = text;
	// for text that shows up in console but not in notify (backported from RTCW)
...
static clientInfo_t *CG_InfoFromScoreIndex(int index, int team, int *scoreIndex) {
	...
	if (cgs.gametype > GT_TOURNAMENT) {
		...
		for (i = 0; i < cg.numScores; i++) {
			if (cg.scores[i].team == team) {
				if (count == index) {
					*scoreIndex = i;
					return &cgs.clientinfo[cg.scores[i].clientNum];
				}
	...
	*scoreIndex = index;
	return &cgs.clientinfo[cg.scores[index].clientNum];
}
		...
		switch (column) {
			case 0:
				if (info->powerups & (1 << PW_REDFLAG)) {
					...
					*handle = cg_items[BG_ItemNumForItem(item)].icon;
				} else if (info->powerups & (1 << PW_NEUTRALFLAG)) {
			...
			case 1:
				...
				break;
			case 2:
				if (Com_ClientListContains(&cg.readyPlayers, sp->clientNum)) {
					return "Ready";
				}
...
Execute the commands in the bind string. Key up events only perform actions if the game key binding is a button command (leading + sign).
These will be processed even in console mode and menu mode, to keep the character from continuing an action started before a mode switch.
=======================================================================================================================================
*/
