...
static void CG_BeamgunTrail(centity_t *cent, vec3_t origin) {
	...
	if (cent->currentState.number == cg.predictedPlayerState.clientNum && cg_antiLag.integer == 2) {
...
static qboolean CG_CalcMuzzlePoint(int entityNum, vec3_t muzzle) {
	...
	if (ps) {
		...
		AngleVectorsForward(ps->viewangles, forward;
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team) {
	...
	if (!weapon->weaponModel || !weapon->flashModel) {
		...
		if (ps) {
			VectorCopy(cg.firstPersonViewOrg, cg.flashOrigin);
			VectorMA(cg.flashOrigin, -8, cg.firstPersonViewAxis[2], cg.flashOrigin);
			VectorCopy(cg.flashOrigin, flash.origin);
		} else {
		...
		if ((cg.predictedPlayerState.eFlags & EF_FIRING) && (ps || cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)) {
...
void CG_NextWeapon_f(int localPlayerNum) {
	...
	for (i = 0; i < MAX_WEAPONS; i++) {
		...
		if (cg.weaponSelect == WP_GAUNTLET && cg_cyclePastGauntlet[localPlayerNum].integer) {
			continue; // never cycle to gauntlet
		}

		if (CG_WeaponSelectable(ps, cg.weaponSelect)) {
			break;
		}
	}
...
void CG_WeaponToggle_f(int localPlayerNum) {
	...
	if (cg.weaponSelect != num) {
		weapon = num;
		oldweapon = cg.weaponSelect;
	} else {
		weapon = cg.weaponToggledFrom;
		oldweapon = WP_NONE;
	}
	...
	cg.weaponToggledFrom = oldweapon;
}
