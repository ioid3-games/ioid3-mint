...
static int CG_MapTorsoToWeaponFrame(playerInfo_t *pi, int frame) {
	...
	// change weapon
	if (frame >= pi->animations[TORSO_DROP].firstFrame && frame < pi->animations[TORSO_DROP].firstFrame + 9) {
		return frame - pi->animations[TORSO_DROP].firstFrame + 6;
	}
	// stand attack
	if (frame >= pi->animations[TORSO_ATTACK].firstFrame && frame < pi->animations[TORSO_ATTACK].firstFrame + 6) {
		return 1 + frame - pi->animations[TORSO_ATTACK].firstFrame;
	}
	// stand attack 2
	if (frame >= pi->animations[TORSO_ATTACK2].firstFrame && frame < pi->animations[TORSO_ATTACK2].firstFrame + 6) {
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles) {
	...
	if (delta < LAND_DEFLECT_TIME) {
		...
		origin[2] += cg.cur_lc->landChange * 0.25 * (LAND_DEFLECT_TIME + LAND_RETURN_TIME - delta) / LAND_RETURN_TIME;
	}
#if 0
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team) {
	...
	if (!weapon->weaponModel || !weapon->flashModel) {
		...
		if ((cg.cur_lc->predictedPlayerState.eFlags & EF_FIRING) && (ps || cg.cur_lc->renderingThirdPerson || cent->currentState.number != cg.cur_lc->predictedPlayerState.playerNum)) {
