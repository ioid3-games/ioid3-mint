...
static void CG_Item(centity_t *cent) {
	...
	wi = NULL;
	// the weapons have their origin where they attach to player models, so we need to offset them or they will rotate eccentrically
...
/*
	...
	if (weapon->missileDlight) {
		trap_R_AddLightToScene(cent->lerpOrigin, weapon->missileDlight, 1.0f, weapon->missileDlightColor[col][0], weapon->missileDlightColor[col][1], weapon->missileDlightColor[col][2], 0);
	}
*/
	// add dynamic light
	if (weapon->missileDlight) {
...
static void CG_CalcEntityLerpPositions(centity_t *cent) {
	...
	// first see if we can interpolate between two snaps for linear extrapolated players
