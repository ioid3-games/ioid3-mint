...
//		{0.2, 1.0, 0.2, 1.0}, {1.0, 0.2, 0.2, 1.0}, {0.5, 0.5, 0.5, 1}};
	...
	if (cent->currentState.weapon) {
		...
		if (value > -1) {
			if (cg.cur_lc->predictedPlayerState.weaponstate == WEAPON_FIRING && cg.cur_lc->predictedPlayerState.weaponTime > 100) {
...
static float CG_DrawPowerups(float y) {
	...
	for (i = 0; i < active; i++) {
		...
		if (item) {
			...
			if (cg.cur_lc->powerupActive == sorted[i] && cg.time - cg.cur_lc->powerupTime < PULSE_TIME) {
...
#define CHATLOC_X 0
	...
	if (cg.cur_lc->teamLastChatPos != cg.cur_lc->teamChatPos) {
		...
		for (i = cg.cur_lc->teamChatPos - 1; i >= cg.cur_lc->teamLastChatPos; i--) {
			CG_DrawString(CHATLOC_X + TINYCHAR_WIDTH, CHATLOC_Y - (cg.cur_lc->teamChatPos - i) * lineHeight, cg.cur_lc->teamChatMsgs[i % chatHeight], UI_TINYFONT, hcolor);
		}
	}
}
...
static void CG_DrawDisconnect(void) {
	...
	if (cmd.serverTime <= cg.cur_ps->commandTime || cmd.serverTime > cg.time) { // special check for map_restart
		return;
	}
...
static void CG_DrawShaderInfo(void) {
	...
	// do a trace that hits all surfaces (including solid and non-solid), and a trace that only hits solid. Use which
	// ever is closer and use solid if 'any contents' trace didn't get a textured surface (such as clip)
