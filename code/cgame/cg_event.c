	...
	if (CG_LocalPlayerState(attacker)) {
		...
		for (i = 0; i < CG_MaxSplitView(); i++) {
			...
			if (cgs.gametype < GT_TEAM) {
				s = va("You fragged %s\n%s place with %i", targetName, CG_PlaceString(ps->persistant[PERS_RANK] + 1), ps->persistant[PERS_SCORE]);
			} else {
	...
	switch (event) {
		...
		case EV_FOOTSTEP:
			...
			if (cg_footsteps.integer) {
				trap_S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[pi->footsteps][rand()&3]);
			}
		...
		case EV_DEATH3:
			...
			if (es->eventParm >= 1) {
				...
				if (cg_blood.integer && cg_gibs.integer) {
					// don't play gib sound when using the kamikaze because it interferes with the kamikaze sound, downside is
					// that the gib sound will also not be played when someone is gibbed while just carrying the kamikaze
