...
static ID_INLINE colorGen_t R_DiffuseColorGen(int lightmapIndex) {
	...
	if (lightmapIndex == LIGHTMAP_NONE) {
		...
		// world; LIGHTMAP_BY_VERTEX, LIGHTMAP_WHITEIMAGE, or a real lightmap already overbright shifted, so use exact
...
Return a shader index for a given world surface.
 lightmapIndex = LIGHTMAP_NONE will create a new shader that is a copy of the one found on the model, without the lighmap stage, if the
							   shader has a lightmap stage.
...
qhandle_t RE_GetSurfaceShader(int surfaceNum, int lightmapIndex) {
	...
	if (lightmapIndex < 0 && surf->shader->lightmapIndex != lightmapIndex) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (shd->stages[i]->rgbGen != CGEN_CONST && shd->stages[i]->rgbGen != CGEN_WAVEFORM && shd->stages[i]->rgbGen != CGEN_COLOR_WAVEFORM) {
...
'lightmapIndex' set to LIGHTMAP_NONE will create a new shader that is a copy of the one found on the model, without the lighmap stage,
if the shader has a lightmap stage.
...
static qboolean ParseStage(shaderStage_t *stage, char **text, int *ifIndent) {
	...
	while (1) {
		...
		// lightmap < name>
			...
			if (!Q_stricmp(token, "$whiteimage") || !Q_stricmp(token, "*white")) {
				...
				bundle->image[0] = R_FindImageFile(token, IMGTYPE_COLORALPHA, IMGFLAG_LIGHTMAP|IMGFLAG_NOLIGHTSCALE|IMGFLAG_NO_COMPRESSION|IMGFLAG_CLAMPTOEDGE);
		...
		// specularReflectance < value>
			...
			if (r_pbr->integer) {
				...
				stage->specularScale[0] = stage->specularScale[1] = stage->specularScale[2] = atof(token);
			}
		}
	...
	// if shader stage references a lightmap but no lightmap is present and shader doesn't specify nolightmap (or also has pointlight), use diffuse cgen
	if (((stage->bundle[0].isLightmap && stage->bundle[0].image[0] == tr.whiteImage) || (stage->bundle[1].isLightmap && stage->bundle[1].image[0] == tr.whiteImage)) && shader.lightmapIndex < 0) {
	...
	// compute state bits
	stage->stateBits = depthMaskBits|blendSrcBits|blendDstBits|atestBits|depthFuncBits|depthTestBits;
...
The current text pointer is at the explicit text definition of the shader. Parse it into the global shader variable. Later functions
will optimize it.
=======================================================================================================================================
*/
static qboolean ParseShader(char **text) {
	...
	while (1) {
		...
		// skyfogvars(< red > < green > < blue >) < density>
			...
			tr.skyFogColorInt = ColorBytes4(fogColor[0], fogColor[1], fogColor[2], 1.0);
...
Check which vertex attributes we only need, so we don't need to submit/copy all of them.
=======================================================================================================================================
*/
...
=======================================================================================================================================
*/
static void CollapseStagesToLightall(shaderStage_t *diffuse, shaderStage_t *normal, shaderStage_t *specular, shaderStage_t *lightmap, qboolean useLightVector, qboolean useLightVertex, qboolean parallax, qboolean tcgen) {
	int defs = 0;
...
static int CollapseStagesToGLSL(void) {
	...
	if (!skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (pStage->multitextureEnv) {
				...
				if (pStage->multitextureEnv != GL_MODULATE || !pStage->bundle[1].isLightmap) {
					skip = qtrue;
					break;
				}
			}
	...
	if (!skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
				...
				switch (pStage2->type) {
					...
					case ST_COLORMAP:
						if (pStage2->bundle[0].tcGen == TCGEN_LIGHTMAP) {
							...
							// Only add lightmap to blendfunc filter stage if it's the first time lightmap is used otherwise it will cause the shader to be darkened by the lightmap multiple times.
							if (!usedLightmap || (blendBits != (GLS_DSTBLEND_SRC_COLOR|GLS_SRCBLEND_ZERO) && blendBits != (GLS_DSTBLEND_ZERO|GLS_SRCBLEND_DST_COLOR))) {
								lightmap = pStage2;
								usedLightmap = qtrue;
							}
						}
			...
			if (diffuse->bundle[0].tcGen == TCGEN_ENVIRONMENT_MAPPED || diffuse->bundle[0].tcGen == TCGEN_ENVIRONMENT_CELSHADE_MAPPED || diffuse->bundle[0].tcGen == TCGEN_LIGHTMAP || diffuse->bundle[0].tcGen == TCGEN_VECTOR) {
				tcgen = qtrue;
			}
				...
				case RC_DRAW_SURFS:
				{
					...
					for (i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs; i++, drawSurf++) {
						...
						if (sortOrder >= newSortOrder) {
							sortOrder++;
							R_ComposeSort(drawSurf, shader->sortedIndex, sortOrder, (entityNum << QSORT_REFENTITYNUM_SHIFT), fogNum, dlightMap, pshadowMap);
						}
					}
...
If vertex lighting is enabled, only render a single pass, trying to guess which is the correct one to best approximate what it is
...
static shader_t *FinishShader(void) {
 ...
 //}
		...
		if ((pStage->stateBits &(GLS_SRCBLEND_BITS|GLS_DSTBLEND_BITS)) && !isLightmap && !hasOpaqueStage) {
...
Given a(potentially erroneous) lightmap index, attempts to load an external lightmap image and/or sets the index to a valid number.
=======================================================================================================================================
*/
...
Dump information on all valid shaders to the console. A second parameter will cause it to print in sorted order.
=======================================================================================================================================
*/
