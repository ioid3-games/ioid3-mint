...
// Changes Globals:		 - 
// ===========================================================================
int ReadFuzzyWeight(int source, fuzzyseparator_t *fs) {
...
// Changes Globals:		 - 
// ===========================================================================
void FreeFuzzySeperators_r(fuzzyseparator_t *fs) {
...
void FreeWeightConfig2(weightconfig_t *config) {
	...
	for (i = 0; i < config->numweights; i++) {
		FreeFuzzySeperators_r(config->weights[i].firstseparator);
...
// Changes Globals:		 - 
// ===========================================================================
fuzzyseparator_t *ReadFuzzySeperators_r(int source) {
	...
	do {
		...
		if (def || !strcmp(token.string, "case")) {
			fs = (fuzzyseparator_t *)trap_HeapMalloc(sizeof(fuzzyseparator_t));
			fs->index = index;
...
// Changes Globals:		 - 
// ===========================================================================
qboolean WriteFuzzyWeight(FILE *fp, fuzzyseparator_t *fs) {
...
// Changes Globals:		 - 
// ===========================================================================
qboolean WriteFuzzySeperators_r(FILE *fp, fuzzyseparator_t *fs, int indent) {
...
qboolean WriteWeightConfig(char *filename, weightconfig_t *config) {
	...
	for (i = 0; i < config->numweights; i++) {
		...
		if (ifw->firstseparator->index > 0) {
			if (!WriteFuzzySeperators_r(fp, ifw->firstseparator, 1)) {
		return qfalse;
	}
		} else {
			if (!WriteIndent(fp, 1)) return qfalse;

			if (!WriteFuzzyWeight(fp, ifw->firstseparator)) {
		return qfalse;
	}
		} // end else
...
// Changes Globals:		 - 
// ===========================================================================
void EvolveFuzzySeperator_r(fuzzyseparator_t *fs) {
...
void EvolveWeightConfig(weightconfig_t *config) {
	...
	for (i = 0; i < config->numweights; i++) {
		EvolveFuzzySeperator_r(config->weights[i].firstseparator);
	}
...
// Changes Globals:		 - 
// ===========================================================================
void ScaleFuzzySeperator_r(fuzzyseparator_t *fs, float scale) {
...
void ScaleWeight(weightconfig_t *config, char *name, float scale) {
	...
	for (i = 0; i < config->numweights; i++) {
		if (!strcmp(name, config->weights[i].name)) {
			ScaleFuzzySeperator_r(config->weights[i].firstseparator, scale);
			break;
		}
	}
...
// Changes Globals:		 - 
// ===========================================================================
void ScaleFuzzySeperatorBalanceRange_r(fuzzyseparator_t *fs, float scale) {
...
void ScaleFuzzyBalanceRange(weightconfig_t *config, float scale) {
	...
	for (i = 0; i < config->numweights; i++) {
		ScaleFuzzySeperatorBalanceRange_r(config->weights[i].firstseparator, scale);
	}
...
// Changes Globals:		 - 
// ===========================================================================
int InterbreedFuzzySeperator_r(fuzzyseparator_t *fs1, fuzzyseparator_t *fs2, fuzzyseparator_t *fsout) {
...
void InterbreedWeightConfigs(weightconfig_t *config1, weightconfig_t *config2, weightconfig_t *configout) {
	...
	for (i = 0; i < config1->numweights; i++) {
		InterbreedFuzzySeperator_r(config1->weights[i].firstseparator, config2->weights[i].firstseparator, configout->weights[i].firstseparator);
	}
