...
qboolean BotBSPModelMinsMaxsOrigin(int modelnum, vec3_t angles, vec3_t mins, vec3_t maxs, vec3_t origin) {
	...
	return qfalse;
} // end of the function BotBSPModelMinsMaxsOrigin
// ===========================================================================
...
void BotAddToAvoidReach(bot_movestate_t *ms, int number, float avoidtime) {
	...
	for (i = 0; i < MAX_AVOIDREACH; i++) {
		if (ms->avoidreach[i] == number) {
			if (ms->avoidreachtimes[i] > trap_AAS_Time()) ms->avoidreachtries[i]++;
...
} // end of the function ProjectPointOntoVector
// ===========================================================================
...
int BotAvoidSpots(vec3_t origin, aas_reachability_t *reach, bot_avoidspot_t *avoidspots, int numavoidspots) {
	...
	for (i = 0; i < numavoidspots; i++) {
		...
		if (squareddist < squaredradius && VectorDistanceSquared(avoidspots[i].origin, origin) > squareddist) {
			...
			if (squareddist < squaredradiuss && VectorDistanceSquared(avoidspots[i].origin, reach->start) > squareddist) {
				type = avoidspots[i].type;
			}
		} // end else
...
#ifdef AVOIDREACH
		// check if it isn't a reachability to avoid for (i = 0; i < MAX_AVOIDREACH; i++) {
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type) {
	...
	if (ms->moveflags & MFL_ONGROUND) {
		...
		if (type & MOVE_JUMP) EA_Jump(ms->playernum);

		if (type & MOVE_CROUCH) EA_Crouch(ms->playernum);
		EA_Move(ms->playernum, hordir, speed);
...
bot_moveresult_t BotTravel_WaterJump(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (dist < 40) EA_MoveUp(ms->playernum);
...
bot_moveresult_t BotTravel_Teleport(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (dist < 30) EA_Move(ms->playernum, hordir, 200);
	else EA_Move(ms->playernum, hordir, 400);
...
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags) {
		...
		if (ent != -1) {
			...
			if (modelnum >= 0 && modelnum < MAX_SUBMODELS) {
				...
				if (modeltype == MODELTYPE_FUNC_PLAT) {
					...
					if ((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_ELEVATOR || // NOTE: the face number is the plat model number
					...
					if ((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_FUNCBOB || // NOTE: the face number is the func_bobbing model number
						(reach.facenum & 0x0000FFFF) != modelnum) {
						reachnum = trap_AAS_NextModelReachability(0, modelnum);
	...
	// if swimming
	if (trap_AAS_Swimming(ms->origin)) ms->moveflags |= MFL_SWIMMING;
	// if against a ladder
	if (trap_AAS_AgainstLadder(ms->origin)) ms->moveflags |= MFL_AGAINSTLADDER;
	...
	if (ms->moveflags & (MFL_ONGROUND|MFL_SWIMMING|MFL_AGAINSTLADDER)) {
		...
		if (reachnum) {
			...
			// special elevator case
				...
				if (ms->areanum == reach.areanum || ms->reachability_time < trap_AAS_Time()) {
					reachnum = 0;
				}
			} else {
				if (bot_developer.integer) {
