...
FIXME: add stuff like "upper rocket launcher", "the rl near the railgun", "lower grenade launcher" etc.
=======================================================================================================================================
*/
...
int BotAddressedToBot(bot_state_t *bs, bot_match_t *match) {
	...
	if (match->subtype & ST_ADDRESSED) {
		...
		if (!BotFindMatch(match->string, &tellmatch, MTCONTEXT_REPLYCHAT) || tellmatch.type != MSG_CHATTELL) {
...
void BotMatch_WhereAreYou(bot_state_t *bs, bot_match_t *match) {
	...
	for (i = 1; i < BG_NumItems(); i++) {
		...
		if (it->giType == IT_HEALTH || it->giType == IT_AMMO || it->giType == IT_HOLDABLE || (it->giType == IT_ARMOR && it->quantity < 50) || (it->giType == IT_TEAM && it->giTag == 0)) {
			continue;
		}
	...
	if (bestitemname) {
		if (gametype == GT_CTF || gametype == GT_1FCTF || gametype == GT_OBELISK || gametype == GT_HARVESTER) {
...
int BotMatchMessage(bot_state_t *bs, char *message) {
	...
	if (!BotFindMatch(message, &match, MTCONTEXT_MISC|MTCONTEXT_INITIALTEAMCHAT|MTCONTEXT_CTF)) {
		return qfalse;
	}
