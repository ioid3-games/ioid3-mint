...
void CopyToBodyQue(gentity_t *ent) {
	...
	if (body->s.groundEntityNum == ENTITYNUM_NONE) {
		...
		VectorCopy(ent->client->ps.origin, body->s.origin2);
...
void ClientSpawn(gentity_t *ent) {
	...
	// run the presend to set anything else, follow spectators wait until all clients have been reconnected after map_restart
...
This should NOT be called directly by any game logic, call trap_DropClient(), which will call this and do server system housekeeping.
=======================================================================================================================================
*/
void ClientDisconnect(int clientNum, qboolean force) {
	gentity_t *ent;
	gentity_t *tent;
	int i;
	...
	level.connections[ent->client->pers.connectionNum].localPlayerNums[ent->player->pers.localPlayerNum] = -1;
