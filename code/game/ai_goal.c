...
itemconfig_t *LoadItemConfig(char *filename) {
	...
	if (!ic->numiteminfo) {
		BotAI_Print(PRT_WARNING, "no item info loaded\n");
	}
...
void BotUnlinkSolidItems(qboolean unlink) {
	...
	for (i = 0, ent = g_entities; i < level.num_entities; i++, ent++) {
		...
		if (ent->s.eType == ET_GENERAL && ent->activator && ent->activator->s.eType == ET_TEAM && (!Q_stricmp(ent->activator->classname, "team_blueobelisk") || !Q_stricmp(ent->activator->classname, "team_redobelisk"))) {
			if (unlink) {
...
void BotDumpAvoidGoals(int goalstate) {
	...
	for (i = 0; i < MAX_AVOIDGOALS; i++) {
		if (gs->avoidgoaltimes[i] >= trap_AAS_Time()) {
			...
			BotAI_Print(PRT_DEVELOPER, "avoid goal %s, number %d for %f seconds", name, gs->avoidgoals[i], gs->avoidgoaltimes[i] - trap_AAS_Time());
		}
	}
}
...
void BotUpdateEntityItems(void) {
	...
	for (ent = BotNextEntity(0); ent; ent = BotNextEntity(ent)) {
		...
		for (li = levelitems; li; li = li->next) {
			...
			if (li->entitynum && li->entitynum == ent) {
				...
				if (ic->iteminfo[li->iteminfo].modelindex != modelindex) {
					...
					if (entinfo.origin[0] != li->origin[0] || entinfo.origin[1] != li->origin[1] || entinfo.origin[2] != li->origin[2]) {
		...
		VectorCopy(entinfo.origin, li->origin);
		// get the item goal area and goal origin
		li->goalareanum = trap_AAS_BestReachableArea(li->origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, li->goalorigin);
...
int BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory, int travelflags, bot_goal_t *ltg, float maxtime) {
	...
	if (ltg) {
		ltg_time = trap_AAS_AreaTravelTimeToGoalArea(areanum, origin, ltg->areanum, travelflags);
	} else {
		ltg_time = 99999;
	}
	// the item configuration
