 ...
 *hintsplit = false;
	...
	if (type < 3) {
		...
		if (brush->mins[type] < dist - PLANESIDE_EPSILON && brush->maxs[type] > dist + PLANESIDE_EPSILON) s = PSIDE_BOTH;
	}
...
side_t *SelectSplitSide(bspbrush_t *brushes, node_t *node) {
	...
	bestvalue = -99999;
	// bestsplits = 0;
	// the search order goes: visible - structural, visible - detail, // nonvisible - structural, nonvisible - detail.
...
// 				value = 5 * facing - 5 * splits;
		...
		if (bestside) {
			...
			if (pass > 0) node->detail_separator = true;	// not needed for vis
			break;
		}
	} (pass = 0;
...
void SplitBrush(bspbrush_t *brush, int planenum, bspbrush_t **front, bspbrush_t **back) {
	...
	*front = *back = NULL;
	...
	if (d_front < 0.2) // PLANESIDE_EPSILON) {	// only on back
		*back = CopyBrush(brush);
		return;
	}

	if (d_back > -0.2) // PLANESIDE_EPSILON) {	// only on front
