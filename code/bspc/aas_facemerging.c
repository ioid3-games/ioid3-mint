...
// if (face1->planenum != face2->planenum) return false;
	...
	if (face1->frontarea != face2->frontarea ||
		...
		if (!face1->frontarea || !face1->backarea || !face2->frontarea || !face2->backarea) return false;
		else if (face1->frontarea != face2->backarea || face1->backarea != face2->frontarea) return false;
// 	return false;
	}
	// this function is to be found in l_poly.c
	neww = TryMergeWinding(face1->winding, face2->winding, mapplanes[face1->planenum].normal);
