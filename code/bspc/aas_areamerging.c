...
int AAS_TryMergeFaceAreas(tmp_face_t *separatingface) {
	...
	for (face1 = tmparea1->tmpfaces; face1; face1 = face1->next[side1]) {
		...
		if (face1->frontarea != tmparea1 && face1->backarea != tmparea1) Error("face does not belong to area1");
		...
		for (face2 = tmparea2->tmpfaces; face2; face2 = face2->next[side2]) {
			...
			// debug: check if the area belongs to the area
			if (face2->frontarea != tmparea2 && face2->backarea != tmparea2) Error("face does not belong to area2");
			// just continue if the face is separating the two areas
			// NOTE: a result of this is that ground and gap areas can
			//      be merged if the separating face is the gap
	...
	// and the other has ground faces(that aren't separating the two areas), // the areas can't be merged
