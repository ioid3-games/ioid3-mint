	...
	for (i = 0; i < brush1->numsides; i++) {
		...
		for (j = 0; j < brush2->numsides; j++) {
			...
			if (side1->planenum == side2->planenum) {
				if (side1->texinfo != TEXINFO_NODE && side2->texinfo != TEXINFO_NODE && side1->texinfo != side2->texinfo) return NULL;
				continue;
			}
	...
	do {
		...
		for (b1 = brushlist; b1; b1 = brushlist) {
			...
			for (b2 = b1->next; b2; b2 = b2->next) {
				...
				if (b1->original->contents != b2->original->contents || b1->original->expansionbbox != b2->original->expansionbbox) newbrush = NULL;
