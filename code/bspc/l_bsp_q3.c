...
/*
	...
	for (i = 0; i < surface->numVerts; i++) {
		...
		if (dot > -0.9 && dot < 0.9 && VectorLength(t1) > 0.1 && VectorLength(t2) > 0.1) {
			break;
		}
	}
...
void Q3_CreatePlanarSurfacePlanes(void) {
	...
	for (i = 0; i < q3bsp->numSurfaces; i++) {
		...
		// Log_Print("normal = %f %f %f, dist = %f\n", q3_surfaceplanes[i].normal[0], // 										q3_surfaceplanes[i].normal[1], // 										q3_surfaceplanes[i].normal[2], q3_surfaceplanes[i].dist);
	}
...
void Q3_SurfacePlane(dsurface_t *surface, vec3_t normal, float *dist) {
	...
	Q3_PlaneFromPoints(q3bsp->drawVerts[surface->firstVert].xyz, q3bsp->drawVerts[surface->firstVert + 1].xyz, q3bsp->drawVerts[surface->firstVert + 2].xyz, normal, dist);
} // end of the function Q3_SurfacePlane*/
// ===========================================================================
...
void Q3_FindVisibleBrushSides(void) {
	...
	// 					|| we == WE_NONCONVEX
				...
				for (k = 0; k < q3bsp->numSurfaces; k++) {
					...
					if (fabs(fabs(plane->dist) - fabs(q3bsp->planes[brushside->planeNum].dist)) > 5) {
						continue;
					}
