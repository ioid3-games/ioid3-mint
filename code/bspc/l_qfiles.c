...
qfile_exttyp_t quakefiletypes[] = {
	...
	{QFILEEXT_AAS, QFILETYPE_AAS}, {NULL, 0}
};
			...
			while (*filter && !found) {
				...
				if (*(filter + 1) == '-' && * (filter + 2) && (*(filter + 2) != ']' || * (filter + 3) == ']')) {
					if (casesensitive) {
						...
						if (toupper(*filename) >= toupper(*filter) && toupper(*filename) <= toupper(*(filter + 2))) found = true;
					} // end else
					filter += 3;
				} else {
					if (casesensitive) {
		...
		for (j = 0; j < globbuf.gl_pathc; j++) {
			...
			if (statbuf.st_mode & S_IFDIR) {
				...
				if (qf) {
					...
					lastqf = qf' || (lastqf->next) lastqf = lastqf->next;
				}
			} // end else
			// 
#ifdef _WIN32
