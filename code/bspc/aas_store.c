...
int AAS_PlaneTypeForNormal(vec3_t normal) {
	...
	// NOTE: epsilon used
	if ((normal[0] >= 1.0 - NORMAL_EPSILON) || (normal[0] <= -1.0 + NORMAL_EPSILON)) return PLANE_X;

	if ((normal[1] >= 1.0 - NORMAL_EPSILON) || (normal[1] <= -1.0 + NORMAL_EPSILON)) return PLANE_Y;

	if ((normal[2] >= 1.0 - NORMAL_EPSILON) || (normal[2] <= -1.0 + NORMAL_EPSILON)) return PLANE_Z;
...
qboolean AAS_GetFace(winding_t *w, plane_t *p, int side, int *facenum) {
	...
	if (face->numedges < 1
#ifdef NOTHREEVERTEXFACES || face->numedges < 3
...
// 	Log_Write("tmparea %d became aasarea %d\r\n", tmparea->areanum, aasarea->areanum);
	...
	//Log_Write("area %d center %f %f %f\r\n", aasworld.numareas, // 				aasarea->center[0], aasarea->center[1], aasarea->center[2]);
