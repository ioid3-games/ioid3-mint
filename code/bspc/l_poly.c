...
// counters are only bumped when running single threaded, because they are an awful coherence problem
...
winding_t *MergeWindings(winding_t *w1, winding_t *w2, vec3_t planenormal) {
	...
	for (i = 0; i < w2->numpoints; i++) {
		...
		for (j = 0; j < numpoints; j++) {
			VectorSubtract(newp[(j + 1)%numpoints], newp[(j)%numpoints], edgevec);
