...
void SV_AddPlayer(client_t *client, int localPlayerNum, const char *infoString) {
	...
	*newplayer = temp;
	clientNum = newplayer - svs.players;
	...
	denied = VM_Call(gvm, GAME_PLAYER_CONNECT, clientNum, qtrue, qfalse, client - svs.clients, localPlayerNum); // firstTime = qtrue
...
quitting or crashing -- SV_FinalMessage() will handle that.
=======================================================================================================================================
*/
void SV_DropPlayer(player_t *drop, const char *reason, qboolean force) {
	...
	challenge_t *challenge;
	int numLocalPlayers;
	int clientNum = drop - svs.players;
	...
	if (!VM_Call(gvm, GAME_PLAYER_DISCONNECT, clientNum, force || numLocalPlayers == 1) && !(force || numLocalPlayers == 1)) {
		return;
	}
...
void SV_PlayerEnterWorld(player_t *player, usercmd_t *cmd) {
	...
	if (client->state == CS_PRIMED) {
		...
		// resend all configstrings using the cs commands since these are no longer sent when the client is CS_PRIMED
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
	...
	if (!cl->download) {
			...
			if (!Q_stricmp(pakptr + 1, "pk3")) {
				...
				for (curindex = 0; curindex < numRefPaks; curindex++) {
					if (!FS_FilenameCompare(Cmd_Argv(curindex), pakbuf)) {
						unreferenced = 0;
						// now that we know the file is referenced, check whether it's legal to download it or if it is a default pak
		...
		if (!(sv_allowDownload->integer & DLF_ENABLE) || (sv_allowDownload->integer & DLF_NO_UDP) || pakType != PAK_FREE || unreferenced || (cl->downloadSize = FS_SV_FOpenFileRead(cl->downloadName, &cl->download)) < 0) {
...
static qboolean SV_ClientCommand(client_t *cl, msg_t *msg) {
	...
	if (seq > cl->lastClientCommand + 1) {
		Com_Printf("Client %s lost %i clientCommands\n", SV_ClientName(cl), seq - cl->lastClientCommand + 1);
...
static void SV_UserMove(client_t *cl, msg_t *msg, qboolean delta) {
	...
	for (lc = 0; lc < MAX_SPLITVIEW; ++lc) {
		...
		// if this is the first usercmd we have received this gamestate, put the player into the world
		if (!player->inWorld) {
		...
		// usually, the first couple commands will be duplicates of ones we have previously received, but the servertimes
...
=======================================================================================================================================
*/
static qboolean SV_ClientIgnoreVoipTalker(const client_t *client, int clientNum) {

	if (client->ignoreVoipFromPlayer[clientNum]) {
		return qtrue;
	}
...
=======================================================================================================================================
*/
static void SV_UserVoip(client_t *cl, msg_t *msg, qboolean ignoreData) {
	...
	clientNum = cl->localPlayers[localPlayerNum] - svs.players;
	...
	for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++) {
		...
		} else if (SV_ClientIgnoreVoipTalker(client, clientNum)) {
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg) {
	...
	// this client has acknowledged the new gamestate so it's safe to start sending it the real time again
