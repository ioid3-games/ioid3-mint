...
void Cvar_VM_Set(const char *var_name, const char *value, const char *vmName) {
	...
	if (flags != CVAR_NONEXISTENT) {
		if (flags & CVAR_PROTECTED) {
			if (value) {
				Com_Error(ERR_DROP, "%s VM tried to set \"%s\" to \"%s\"", vmName, var_name, value);
			} else {
				Com_Error(ERR_DROP, "%s VM tried to modify \"%s\"", vmName, var_name);
			}
		...
		if (((flags & CVAR_CHEAT) && !cvar_cheats->integer) || ((flags & CVAR_LATCH) && !(flags & CVAR_VM_CREATED)) || ((flags & CVAR_SYSTEMINFO) && CL_ConnectedToRemoteServer())) {
			force = qfalse;
		}
	}
...
void Cvar_SetCheatState(void) {
	...
	for (var = cvar_vars; var; var = var->next) {
		if (var->flags & CVAR_CHEAT) {
			// the CVAR_LATCHED|CVAR_CHEAT vars might escape the reset here because of a different var->latchedString
			if (var->latchedString) {
				Z_Free(var->latchedString);
				var->latchedString = NULL;
			}
...
void Cvar_Toggle_f(void) {
	...
	if (c == 2) {
		Cvar_User_Set(Cmd_Argv(1), va("%d", !Cvar_VariableValue(Cmd_Argv(1))));
		return;
	}
...
void Cvar_Restart(qboolean unsetVM) {
	...
	while (curvar) {
		// custom reset (default value) are reloaded in FS_Restart before this function is run in Com_GameRestart. Any user cvars with
		// custom reset are for the new fs_game, so don't remove them
