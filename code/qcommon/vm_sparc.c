...
/**************************************************************************************************************************************
 This code is based almost entirely upon the vm_powerpc.c code by Przemyslaw Iskra. All I did was make it work on Sparc :-) -DaveM.
**************************************************************************************************************************************/
	...
	for (i = 0; op->args[i] != ARG_NONE; i++) {
		...
		switch (op->args[i]) {
			...
			case ARG_RD:
				...
				break;
			case ARG_SIMM13:
				insn |= SIMM13(val); vimm(val, 13, 0, 1, i);
				break;
			case ARG_DISP30:
				insn |= DISP30(val); vimm(val, 30, 0, 1, i);
				break;
			case ARG_IMM22:
				insn |= IMM22(val); vimm(val, 22, 0, 0, i);
				break;
			case ARG_DISP22:
				insn |= DISP22(val); vimm(val, 22, 0, 1, i);
				break;
			case ARG_SWTRAP:
				insn |= SWTRAP(val); vimm(val, 7, 0, 0, i);
				break;
		}
	}
...
static void pgreg(int reg_num, int arg_index, int flt) {
	...
	if (!flt) {
		...
		Com_Printf("%s%s%d", (arg_index ? ", " : ""), fmt[reg_num >> 3], reg_num & 7);
	} else {
...
static void pimm(unsigned int val, int bits, int shift, int sgned, int arg_index) {
	...
	if (sgned) {
		...
		sval >>= (32 - bits);
		Com_Printf("%s%d", (arg_index ? ", " : ""), sval);
	} else {
		Com_Printf("%s0x%08x", (arg_index ? ", " : ""), val);
	}
}
...
static void sparc_disassemble(unsigned int insn) {
	...
	for (op_idx = 0; op_idx < SPARC_NUM_OPCODES; op_idx++) {
		...
		for (i = 0; op->args[i] != ARG_NONE; i++) {
			switch (op->args[i]) {
				case ARG_RS1:
					pgreg((insn >> 14) & 0x1f, i, flt);
					break;
				case ARG_RS2:
					pgreg((insn >> 0) & 0x1f, i, flt);
					break;
				case ARG_RD:
					pgreg((insn >> 25) & 0x1f, i, rd_flt);
					break;
				case ARG_SIMM13:
					pimm(insn, 13, 0, 1, i);
					break;
...
static void blockcopy(unsigned int dest, unsigned int src, unsigned int count) {
	...
	if ((dest & dataMask) != dest || (src & dataMask) != src || ((dest + count) & dataMask) != dest + count || ((src + count) & dataMask) != src + count) {
...
static void sparc_compute_code(vm_t *vm, struct func_info * const fp) {
...
#if 0
	{
		...
		for (i = 0; i < data->codeLength / 4; i += 8) {
			Com_Printf("\t.word\t0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", insn[i + 0], insn[i + 1], insn[i + 2], insn[i + 3], insn[i + 4], insn[i + 5], insn[i + 6], insn[i + 7]);
		}
	}
#endif
	dhp = fp->data_first;
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
	...
	for (i_count = 0; i_count < header->instructionCount; i_count++) {
		...
		if (op == OP_CVIF || op == OP_CVFI || (op == OP_LEAVE && (opi & opArgF))) {
			fi.need_float_tmp = 1;
		}
