...
// The operands table. The fields are bitm, shift, insert, extract, flags.
static const struct powerpc_operand powerpc_operands[] = {
...
// The BD field in a B form instruction when the - modifier is used. This sets the y bit of the BO field appropriately.
#define BDM BD + 1
	{0xfffc, 0, insert_bdm, PPC_OPERAND_RELATIVE|PPC_OPERAND_SIGNED},
...
// An optional BF field. This is used for comparison instructions, in which an omitted BF field is taken as zero.
#define OBF BF + 1
...
#define DS D + 1
	{0xfffc, 0, NULL, PPC_OPERAND_PARENS|PPC_OPERAND_SIGNED|PPC_OPERAND_DS},
...
// The FRS field in an X form instruction or the FRT field in a D, X or A form instruction.
#define FRS FRC + 1
#define FRT FRS
...
The RA field in a D or X form instruction which is an updating store or an updating floating point load, which means that the RA field
may not be zero.
=======================================================================================================================================
*/
...
The RB field in an X form instruction when it must be the same as the RS field in the instruction. This is used for extended mnemonics
like mr. This operand is marked FAKE. The insertion function just copies the BT field into the BA field, and the extraction function
just checks that the fields are the same.
=======================================================================================================================================
*/
...
// A B form instruction setting the BO field and the condition bits of the BI field.
...
// An XL form instruction which explicitly sets the y bit of the BO field.
#define XLYLK(op, xop, y, lk) (XLLK((op), (xop), (lk))|((((unsigned long)(y)) & 1) << 21))
#define XLYLK_MASK (XL_MASK|Y_MASK)
// An XL form instruction which sets the BO field and the condition bits of the BI field.
...
// An XFX form instruction with the SPR field filled in except for the SPRG field.
...
#define BOU (0x14)
// The BI condition bit encodings used in extended conditional branch mnemonics.
#define CBLT (0)
#define CBGT (1)
#define CBEQ (2)
#define CBSO (3)
...
#define TONE (0x18)
#define TOU (0x1f)
// Smaller names for the flags so each entry in the opcodes table will fit on a single line.
#undef PPC
