	...
	while (instruction < header->instructionCount) {
		...
		switch (op) {
			...
			case OP_GEF:
				...
				// codeBase[pc] is the instruction index. Convert that into an offset into the int-aligned codeBase[] by the lookup table.
...
#ifdef DEBUG_VM
					// this is just our stack frame pointer, only needed for debugging
