...
- base game is a variable to allow mods based on other mods (such as baseq3 + missionpack content combination in a mod for instance)
		...
		if (search->pack->hashTable[hash]) {
			...
			do {
				...
				if (!FS_FilenameCompare(pakFile->name, filename)) {
					...
					// shaders, txt, arena files by themselves do not count as a reference as these are loaded from all pk3s
					// from every pk3 file..
					len = strlen(filename);
