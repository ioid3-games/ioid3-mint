...
char *CL_JoyEventToString(const joyevent_t *event) {
	...
	switch (event->type) {
		case JOYEVENT_AXIS:
			Com_sprintf(str, sizeof(str), "%saxis%d", (event->value.axis.sign == -1) ? "-" : "+", event->value.axis.num);
			break;
		case JOYEVENT_BUTTON:
			Com_sprintf(str, sizeof(str), "button%d", event->value.button);
			break;
		case JOYEVENT_HAT:
...
void Cmd_JoyRemapList_f(void) {
	...
	for (i = 0; i < MAX_JOY_REMAPS; i++) {
		...
		Com_Printf("%s %s\n", CL_JoyEventToString(&device->remap[i].event), Key_KeynumToString(device->remap[i].keynum));
	}
}
#if 0
/*
