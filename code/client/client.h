...
#include <opus.h>
#endif
// Lower this define to change max supported local clients for client/renderer,  cgame gets max from client when it's initialized.
...
typedef struct {
	...
	int serverCommandNum;	// execute all commands up to this before making the snapshot current
} clSnapshot_t;

/**************************************************************************************************************************************

	The clientActive_t structure is wiped completely at every new gamestate_t, potentially several times during an established
	connection.
...
typedef struct {
	...
	int mouseIndex;
	vec3_t viewAngles;		// used so cgame can save view angles across vid_restart. It is cleared to 0 upon entering each level.
} clientActivePlayer_t;

/**************************************************************************************************************************************

	The parseEntities array must be large enough to hold PACKET_BACKUP frames of entities, so that when a delta compressed message
	arives from the server it can be un-deltad from the original.
...
typedef struct {
	...
	usercmd_t cmdss[CL_MAX_SPLITVIEW][CMD_BACKUP];	// each message will send several old cmds
...
/**************************************************************************************************************************************
	...
	The clientConnection_t structure is wiped when disconnecting from a server, either to go to a full screen console, play a demo, or
...
typedef struct {
	...
	// message sequence is used by both the network layer and the delta compression layer
	...
	// outgoing data...
	// if voipTargets[i / 8] & (1 << (i % 8)), then we are sending to playerNum i.
...
/**************************************************************************************************************************************
	...
	The clientStatic_t structure is never wiped, and is used even when no client connection is active at all (except when CL_Shutdown
	is called).
