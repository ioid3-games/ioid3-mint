...
sets clc.voipTargets according to cl_voipSendTarget. Generally we don't want who's listening to change during a transmission, so this
is only called when the key is first pressed.
=======================================================================================================================================
*/
	...
	if ((cl_voipSend->integer) || (finalFrame)) { // user wants to capture audio?
		...
		if (samples >= packetSamples) {
			...
			if ((useVad) && (clc.voipPower[clc.playerNums[0]] < cl_voipVADThreshold->value)) {
				...
				if (encio != NULL) {
					fwrite(clc.voipOutgoingData, bytes, 1, encio); fflush(encio);
				}
				...
				if (decio != NULL) {
					fwrite(sampbuffer, voipFrames * VOIP_MAX_FRAME_SAMPLES * 2, 1, decio); fflush(decio);
				}
#endif
			}
		}
...
qboolean CL_CheckPaused(void) {
	...
	// if cl_paused->modified is set, the cvar has only been changed in this frame. Keep paused in this frame to ensure the server doesn't
	// lag behind
