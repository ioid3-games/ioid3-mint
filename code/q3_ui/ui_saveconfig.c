...
static void UI_SaveConfigMenu_SaveEvent(void *ptr, int event) {
	...
	COM_StripExtension(MField_Buffer(&saveConfig.savename.field), configname, sizeof(configname));
...
static void UI_SaveConfigMenu_SavenameDraw(void *self) {
	...
	CG_FillRect(f->generic.x, f->generic.y, f->field.widthInChars * SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, colorBlack);
	MField_Draw(&f->field, f->generic.x, f->generic.y, style, color, (style & UI_PULSE));
}
...
static void UI_SaveConfigMenu_Init(void) {
	...
	saveConfig.banner.color = text_banner_color;
